...
- <a href="#complemento_3">Complemento 3: Custom Pipes</a>
...
### `Complemento 3: Custom Pipes` <a name="complemento_3"></a>

Crie um Pipe customizado em NestJS para ser utilizado em uma rota qualquer do seu projeto.
...
├── validators/
│   ├── games-exist.validator.ts
│   └── order-status-transition.validator.ts
├── app.module.ts
└── main.ts
...
### `Custom Pipe: OrderStatusPipe` <a name="order_status_pipe"></a>

**Implementação:**

```
// src/pipes/order-status.pipe.ts
...
- Implementação de um pipe personalizado para validação de enumerações;
- Garantia de que o valor recebido seja um membro válido da enumeração _OrderStatusEnum_;
- Tratamento de exceções personalizado para valores inválidos;
- Transformação de strings para maiúsculas para padronização;

<br>

**Uso:**
Foi criado para ser utilizado na controller de Orders para os métodos _findOrderByStatus_ e _updateOrderStatus_.
